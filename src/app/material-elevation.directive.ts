...
export class MaterialElevationDirective implements OnChanges {
  private NAME_OF_CUSTOM_CLASS = 'selected-card';
  @Input()
  defaultElevation = 2;
  ...
  setElevation(amount: number) {   
    ...
    // add the given elevation class + //shadow class
    const newClass = `mat-elevation-z${amount}`;
    this.renderer.addClass(this.element.nativeElement, newClass);
    //16 -> hover on
    if(amount == 16){
      this.renderer.addClass(this.element.nativeElement,this.NAME_OF_CUSTOM_CLASS);
    }else{
      this.renderer.removeClass(this.element.nativeElement, this.NAME_OF_CUSTOM_CLASS);
